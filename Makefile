...
# toolchain). Compares against a recorded baseline
# (expected/tiger64-riscv64.txt) rather than demanding 100% - see that file
# for the current pass/fail count, and tests/tiger64/README for the known
# gaps behind any failures. Still in test-all below, like every other tier
# here.
...
# needs the alpha cross toolchain). See expected/tiger64-alpha.txt for the
# current pass/fail count and tests/tiger64/README for the known gaps.
test-tiger64-alpha::
...
# x86 on an x86 host). Unlike the Mach-O tier below, this one is NOT
# Darwin-gated: it is the whole point of this tier (verified end to end from
# fork-tiger per the "runtime: split arm64cont.s/amd64cont.s" commit), so it
# always runs in test-all - the arm64/amd64 cross toolchains are already a
# test-all prerequisite via test-tiger64-riscv64's own sibling requirements.
# See expected/tiger64-arm64.txt for the current pass/fail count.
test-tiger64-arm64::
...
# aarch64-linux dev host). Not Darwin-gated, same reasoning as
# test-tiger64-arm64 above. See expected/tiger64-amd64.txt for the current
# pass/fail count and docs/claude_notes/notes_amd64.txt for the known gaps.
test-tiger64-amd64::
...
# on an Apple Silicon Mac). Unlike the ELF backends above, this tier's own
# checked-in tigermain-arm64-mach-o.o/stdlib-arm64-mach-o.a can only be
# regenerated BY HAND on a real Mac (see regenerate-arm64-mach-o.sh's own
# header) - re-run that script plus "run-tiger64-arm64-mach-o.sh --update"
# after any fork-tiger runtime/stdlib change, or this tier's baseline will
# silently lag its ELF sibling's (expected/tiger64-arm64.txt). Darwin-only
# (see -arm64-mach-o's own comment in driver/main.ml) - test-all below now
# only pulls this tier in when ./configure detected Darwin (CCARM64MACHO set
# in Makefile.config), so a plain "make test-all" on Linux no longer fails
# outright for lack of an ld64/Mach-O linker; run "make
# test-tiger64-arm64-mach-o" directly to force it anyway (still fails on
# Linux, same reason). See expected/tiger64-arm64-mach-o.txt for the current
# pass/fail count.
...
# output runs under Rosetta 2). Same by-hand regeneration caveat as
# test-tiger64-arm64-mach-o above - see docs/claude_notes/notes_amd64.txt
# for the known gaps, including the real, general amd64 register-allocation
# correctness bug in Post.binop that was the big fix here. This compares
# against a recorded baseline (expected/tiger64-amd64-mach-o.txt) rather
# than demanding 100%, so it still catches regressions same as the rest.
