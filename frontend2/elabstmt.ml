...
let elab_functions validate srcmap r env =
  ...
  (*s: elaboration of continuation formals *)
  ...
  (*e: elaboration of continuation formals *)
  let rec full_stmt r reachable s =
    ...
    let assign lhs rhs =
      ...
      with Invalid_argument _ -> 
       (*s: complain about length mismatch in assignment *)
    ...
    let return conv alt args =
      ...
      E.ematch3 i n args (fun i n args ->
        if i >= 0 && n >= 0 && i <= n then
          ...
          (*s: raise error exn with bad continuation numbers *)
          if i < 0 then
