...
# -amd64, run under Rosetta 2 translation (this machine is arm64-apple-
# darwin, not x86_64-apple-darwin - no qemu, no explicit wrapper command: a
# plain x86_64 Mach-O executable just runs, Rosetta 2 translation is
# transparent at exec() time - already confirmed by demos/hello_amd64.c--'s
# own milestone), no -static: Apple does not support static-linking
# libSystem), and checked against stdout/exit code. The amd64 counterpart of
# run-tiger64-arm64.sh - see that script first, this one only differs in the
# target (and in needing the explicit "-arch x86_64" cross-target flag,
# since this host does not natively match the target - see
...
# run-tiger64-arm64.sh's own, so this needs no fork-tiger checkout to run.
...
# claude: $CCAMD64 is two words ("clang -arch x86_64") - "command -v" only
# checks the first word (the actual executable). NOT "set -- $CCAMD64": this
# script still needs its own positional parameters ($1 = "--update", "$@"
# for the test-name filter) below - "set --" would clobber them with
# $CCAMD64's own words instead. Parameter-expansion strips the first word
# without touching "$@" at all.
ccamd64_prog=${CCAMD64%% *}
if ! command -v "$ccamd64_prog" >/dev/null 2>&1; then
  echo "run-tiger64-amd64.sh: no $ccamd64_prog (Xcode command line tools not installed?)" >&2
  exit 2
fi
if [ ! -f "$LIB" ]; then
...
while read -r name src rc stdin_file; do
  ...
  if ! $CCAMD64 "$T/tigermain-amd64.o" "$B/$name.o" "$T/stdlib-amd64.a" \
