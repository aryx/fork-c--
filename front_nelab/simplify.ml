...
module Safe = struct
    (*s: RTL operator implementations *)
    let default op w args = RP.App((op,w),args)
    (*x: RTL operator implementations *)
    ...
    (*s: cases for rewriting aggregates in big-endian or little-endian memory *)
    | [w;n],[
       (*s: little-endian memory fetch *)
       RP.Fetch(
         (*s: little-endian memory location *)
         RP.Mem ((sp, Rtl.LittleEndian, mcell) as mspace, ct, addr, assn)
         (*e: little-endian memory location *)
          , w')
       (*e: little-endian memory fetch *)
       ] when Cell.divides mcell n ->
        RP.Fetch (RP.Mem (mspace, Cell.to_count mcell n, addr, assn), n)
    | [w;n],[
        (*s: big-endian memory fetch *)
        RP.Fetch(
            (*s: big-endian memory location *)
            RP.Mem ((sp, Rtl.BigEndian, mcell) as mspace, ct, addr, assn)
            (*e: big-endian memory location *)
           , w'')
        (*e: big-endian memory fetch *)
    ...
    | [w;n],[RP.App(("shrl", [_]), [
        ...
        if Cell.divides mcell shamt then
          ...
          default "lobits" ws args
    (*e: cases for rewriting aggregates in big-endian or little-endian memory *)
    (*s: cases for rewriting aggregates in big-endian or little-endian registers *)
    | [w;n],[
       (*s: little-endian register fetch *)
       RP.Fetch(
          (*s: little-endian register location *)
          RP.Reg ((sp, Rtl.LittleEndian, rcell) as rspace, index, count)
          (*e: little-endian register location *)
          , w'')
       (*e: little-endian register fetch *)
       ] when Cell.divides rcell n ->
        RP.Fetch (RP.Reg (rspace, index, Cell.to_count rcell n), n)
    | [w;n],[
       (*s: big-endian register fetch *)
       RP.Fetch(
          (*s: big-endian register location *)
          RP.Reg ((sp, Rtl.BigEndian,    rcell) as rspace, index, count)
          (*e: big-endian register location *)
          , w'')
       (*e: big-endian register fetch *)
    ...
    | [w;n],[RP.App(("shrl", [_]), [
        ...
        if Cell.divides rcell shamt then
          ...
          default "lobits" ws args
    (*e: cases for rewriting aggregates in big-endian or little-endian registers *)
    | [i;j], [x] when i = j -> x
    ...
    (*e: RTL operator implementations *)
    (*s: traverse RTL *)
