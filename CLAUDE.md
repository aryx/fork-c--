...
regression test. A `-ppc` backend exists too, verified the same way via
...
./configure                      # required once (writes Makefile.config,
                                  # driver/Config.ml - see below)
dune build                       # or: make
./bin/qc -driver_parse demos/hello.c--
```

`./configure` also detects, per backend, a real cross-compiler for this
host and bakes it into `driver/Config.ml` (gitignored, regenerated by
every `./configure` run) so `qc`'s own `-as`/`-ld` (and `-print-cc`, for
a client toolchain like fork-tiger's own `configure`) use a host-verified
toolchain rather than a hardcoded guess — `driver/CLI.ml` used to default
to one (e.g. plain `clang -target i386-unknown-linux-gnu` for `-x86`),
removed after it turned out to fail to even statically link on an
aarch64-linux host with no i386 sysroot; `effective_cc` now fails loudly
instead of guessing when `./configure` found nothing for a backend (pass
`-as`/`-ld`/`QC_AS`/`QC_LD` explicitly in that case). The two Mach-O/
Darwin-only backends (`-arm64-mach-o`/`-amd64-mach-o`) are the exception:
plain `clang` needs no sysroot workaround there, so they keep a real
default and `./configure` doesn't probe for them at all. See
