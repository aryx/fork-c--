...
(* claude: no upstream riscv32asm.nw exists to port - modeled on
...
object (this)
    ...
    method section name =
         ...
         * "a",@progbits (not ARM's %progbits - riscv32-linux-gnu-as's
    ...
    method org n    = unimp "no .org in riscv32 assembler"
    ...
    method value (v:Bits.bits) = match Bits.width v with
        ...
        | w ->  unimp (sprintf "unsupported width %d in riscv32 assembler" w)
    ...
    method longjmp_size () =
      Impossible.unimp "longjmp size not set for riscv32 -- needed for alternate returns"

    method private instruction rtl =
        output_string _fd (Riscv32rec.to_string rtl);
        output_string _fd "\n"
    ...
    method cfg_instr (cfg, proc) =
         ...
         * ra_offset=4 (see riscv32.ml) assumes every jal/jalr we emit is
