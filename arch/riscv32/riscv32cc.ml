...
(* Purpose *)
(*****************************************************************************)
(* The RISC-V (RV32) calling-convention automata - the (string *
 * Automaton.cc_spec) table riscv32.ml's T.cc_specs needs and that
 * Riscv32call.cconv's "string" argument looks entries up in.
 *
 ...
 *                     riscv32call.ml's jump_tgt_reg) - NOT in vol_int/
 *                     nvl_int, same role as mips.ml's $1/at, arm.ml's r12/ip
 *   x6-x7     t1-t2 volatile temporaries
 *   x8        s0/fp reserved, NOT in nvl_int - riscv32-linux-gnu-gcc's
 ...
 * No floating point (riscv32.ml's Post.unrm/binrm are still
...
let r i = (Riscv32regs.rspace, i, Rtl.C 1)

(* keep in sync with Riscv32call.sp_align, which is not exported *)
let sp_align = 16
...
let c_call =
  widen_multiple 32 *> useregs (List.map r (Auxfuns.from 10 ~upto:17)) *> overflow_up

let c_results =
  widen_multiple 32 *> useregs [ r 10 ]
...
let c_cutto =
  widen_exact 32 *> useregs (List.map r (Auxfuns.from 18 ~upto:25)) *> overflow_up
...
(* Entry point *)
(*****************************************************************************)
(* Every name here must be handled by Riscv32call.cconv's own match (it also
