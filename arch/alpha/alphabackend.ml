...
(* Purpose *)
(*****************************************************************************)
(* The Alpha back-end phase pipeline - designed from scratch, same
 * situation arch/sparc/sparcbackend.ml was in (no Alpha.layout anywhere,
 * not even in upstream's dead Lua - see that file's header for the same
 * story).
 *
 * Read arch/ppc/ppcbackend.ml first for the general shape (this follows
 * it almost exactly: single layout function, no per-calling-convention
 * dispatch). The frame is simpler than PPC's Mach-O one and simpler than
 * SPARC's windowed one: the DEC Alpha ABI has no fixed "linkage area"
 * a caller must always reserve (unlike Mach-O PowerPC's 24-byte one) and
 * no register-window save area (unlike SPARC's 96-byte one, hardware-
 * rotated on every call) - a frame is just the incoming/outgoing
 * argument-overflow areas Alphacc.ml's automata actually compute, plus
 * the usual spills/continuations/stack-data in between. If a variadic
 * callee ever needs its own a0-a5 flush area (the same reason PPC always
 * reserves 32 bytes), that would be added here once something actually
 * exercises it - hello.c-- and the tiger runtime do not.
 *)
...
let optimizer ~opt_level ~regalloc (asm : Ast2ir.proc Asm.assembler) (proc : Ast2ir.proc) : unit =
  ...
  dump_cfg "instrsel-cfg" "BEFORE instruction selection (alpha):" proc;
  let proc = run Alpha.X.cfg proc in
  dump_cfg "instrsel-cfg" "AFTER instruction selection (alpha):" proc;
