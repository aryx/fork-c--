...
# 1 "riscv64rec.mlb"
  ...
  (* claude: no upstream riscv64rec.nw exists to port - qc-- predates
   * RISC-V (see riscv64.ml's header comment). Modeled on
   ...
   * actually needed by demos/hello_riscv64.c-- is covered: base+offset
  ...
  (* claude: signed decimal, not unsigned - riscv64-linux-gnu-as's addi/lw/
  ...
  (* claude: RISC-V load/store width suffixes - b/h/w/d, and (like MIPS,
   * unlike ARM) a zero-extending sub-word load just appends "u" to the
   * same suffix (lbu/lhu/lwu) rather than using a distinct mnemonic. *)
  let suffix = function
      | 8  -> "b"
      | 16 -> "h"
      | 32 -> "w"
