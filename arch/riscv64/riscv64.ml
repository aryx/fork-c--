...
(* claude: RISC-V (RV64, 64-bit little-endian) target description. No
 * upstream riscv.nw exists to port - qc-- predates RISC-V entirely, so this
 * whole backend is new development, same situation arch/arm/arm.ml's
 * surrounding files were in (see notes_arm.txt).
 *
 * Architecturally this is closer to MIPS than to ARM/SPARC/Alpha: RISC-V has
 * NO condition-code register at all - beq/bne/blt/bge/bltu/bgeu compare two
 * registers directly and branch, exactly the shape mips.ml's Post.bc_guard
 * already models (see its own comment on why sparcrec.mlb/alpharec.mlb, not
 * mipsrec.mlb, were the templates for ARM's cc-based branches - here it is
 * the other way around: mips.ml IS the template). Where this differs from
 * MIPS: RISC-V has no branch-delay slot (like ARM/Alpha/PPC, unlike MIPS/
 * SPARC), so do_call/cut_to follow arm.ml's ordering discipline (sp update
 * before the jump that leaves control, not "in the delay slot" after it),
 * and registers/memory are a uniform 64 bits wide (like alpha.ml, via
 * Space.Standard64, not mips.ml's Standard32).
 *)
open Nopoly
let arch        = "riscv64"                    (* architecture *)
let wordsize    = 64
module A  = Automaton
...
module Post = struct
     ...
     * instruction by riscv64rec.mlb's grammar (jal/jalr's rd := pc+4 is a
...
let target =
    ...
    ; T.cc_specs            = Riscv64cc.cc_specs
    ; T.cc_spec_to_auto     = Riscv64call.cconv
