...
(* Purpose *)
(*****************************************************************************)
(* The RISC-V (RV64) back-end phase pipeline - designed from scratch, same
 ...
 * argument-overflow areas Riscv64cc.ml's automata actually compute, plus
...
let optimizer ~opt_level ~regalloc (asm : Ast2ir.proc Asm.assembler) (proc : Ast2ir.proc) : unit =
  ...
  let proc = run (Placevar.context Riscv64.placevars) proc in
  ...
  dump_cfg "instrsel-cfg" "BEFORE instruction selection (riscv64):" proc;
  let proc = run Riscv64.X.cfg proc in
  dump_cfg "instrsel-cfg" "AFTER instruction selection (riscv64):" proc;
