...
(* Purpose *)
(*****************************************************************************)
(* The ARM back-end phase pipeline - designed from scratch, same situation
 * arch/mips/mipsbackend.ml/arch/sparc/sparcbackend.ml/arch/alpha/
 * alphabackend.ml were in (no Arm.layout anywhere - upstream never had one,
 * see arch/arm/arm.ml's own header comment: upstream's src/ only ever had
 * arm.nw, the target-description module alone).
 *
 ...
 * SPARC's windowed one: AAPCS has no fixed "linkage area" a caller must
 * always reserve and no register-window save area - a frame is just the
 * incoming/outgoing argument-overflow areas Armcc.ml's automata actually
 * compute, plus the usual spills/continuations/stack-data in between.
 *)
 ...
 * Same shape/gating as arch/mips/mipsbackend.ml's/arch/alpha/
...
let optimizer ~opt_level (asm : Ast2ir.proc Asm.assembler) (proc : Ast2ir.proc) : unit =
  ...
  let proc = run (Placevar.context Arm.placevars) proc in
  ...
  dump_cfg "instrsel-cfg" "BEFORE instruction selection (arm):" proc;
  let proc = run Arm.X.cfg proc in
  dump_cfg "instrsel-cfg" "AFTER instruction selection (arm):" proc;
